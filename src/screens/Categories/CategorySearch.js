...
class CategorySearch extends Component {
    ...
    return (
      <View style={{ height: '85%', backgroundColor: '#f3f3f3' }}>
        <Grid
          ...
          keyExtractor={(product, index) => `product-${product.id}-preview-index-${index}`}
          renderItem={product => (
            <STCartGridItem
              item={product.item}
  ...
  clickAutocompleteItem(value)
  {
      this.searchChange(value.name);
  ...
  renderAutoComplete = () => {
    ...
    return (
      <View style={{position:'absolute',flexDirection:'row',top:60,marginHorizontal:15}}>
        <View style={{flex:1,backgroundColor:'#e0e0e0',borderRadius:10}}>
          {
            products.map((value, i) => (
              <TouchableOpacity
                key={`autocomplete-product-${value.id}`}
                onPress={() => this.clickAutocompleteItem(value)}
              >
                  <Text style={{ color: '#000',paddingVertical:10,paddingHorizontal:10}}>{value.name}</Text>
              </TouchableOpacity>
            ))
          }
        </View>
  ...
  render() {
    return (
      <View style={{ flex: 1 }}>
        ...
        { this.state.showAutoComplete && this.renderAutoComplete()}
      </View>
    );
  }
  ...
  fetchSearch = (keyword) => this.props.searchCategory({search:keyword})
  searchSubmit = () => {
    this.setState({ currentPage: 1 });
    setTimeout(() => {
      this.fetchProducts();
...
const mapDispatchToProps = (dispatch) => ({
  searchCategory: (forms) => dispatch(ProductTypes.search(forms)),
  emptySearch: () => dispatch(ProductTypes.clearSearch()),
})
