...
class AppOnboarding extends Component {
  ...
  constructor(props) {
    ...
    OneSignal.init(Config.ONE_SIGNAL_APP_ID); //91e8224d-c5ed-4f54-b2ca-9d170a18b072
  ...
  renderPageView(item) {
    ...
    const displayContinue = !this.props.onboarding || this.props.onboarding.length === (activeIndex + 1);
  ...
  render() {
    ...
    if ((!Array.isArray(onboarding) || onboarding.length === 0) && this.props.initialized) {
        ...
        <Spinner visible={this.state.isLoading} />
        {
          onboarding && (
            <Swiper
              ...
              loop={false}
            >
              { onboarding.map((item, i) => this.renderPageView(item)) }
            </Swiper>
          )
        }
      </View>
